CShaderStore = class("CShaderStore");

function CShaderStore:initialize( )
	self.m_pShaders 		= { };
	self.m_pShadersCode 	= { };
end;

function CShaderStore:ToStore( sIndex, sCode )
	self.m_pShadersCode [ sIndex ] = sCode;
end;

function CShaderStore:FromStore( sIndex )
	return self.m_pShadersCode [ sIndex ];
end;

function CShaderStore:Create( sIndex, ... )
	if ( self.m_pShadersCode [ sIndex ] ) then
		local pShader = DxShader( self.m_pShadersCode [ sIndex ], ... );
		if ( pShader ) then
			return pShader;
		end
	end
...
g_pShaderStore = CShaderStore( );

-- add shaders
g_pShaderStore:ToStore("tx_replacer", [[
//
// mta-helper.fx
//
...
float4 f4Diffuse = (float4)1.0;

bool bStaticPreLight = false;

sampler gTextrueSampler = sampler_state
{
	Texture = (gTexture);
};
...
{    
    float4 texColor = tex2D( gTextrueSampler, PS.TexCoord );
	
	if ( !bStaticPreLight ) 
	{
		PS.Diffuse *= f4Diffuse;
	}
	else
	{
		PS.Diffuse = f4Diffuse;
...
g_pShaderStore:ToStore("tx_rainbow", [[
float Time : TIME;
...
-- rainbow shader
g_pRainBowShader = g_pShaderStore:Create("tx_rainbow", 0, 0, true);